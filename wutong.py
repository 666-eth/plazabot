...
def create_web3_with_proxy(proxy=None):
    if proxy:
        return Web3(
            HTTPProvider('https://sepolia.base.org', request_kwargs={'proxies': {'http': proxy, 'https': proxy}}))
...
rewardContractAddress = Web3.to_checksum_address('0xb01866f195533de16eb929b73f87280693ca0cb4')
...
async def claim_faucet(address, proxy=None):
    try:
        ...
        print(
            f"{Fore.BLUE}已成功为 {address} 申请水龙头{'（使用代理: ' + proxy + '）' if proxy else ''}{Style.RESET_ALL}")
        print(f"{Fore.RED}申请响应: {response.json()}{Style.RESET_ALL}")
    except requests.exceptions.ProxyError as e:
        print(
            f"{Fore.RED}代理连接失败: {str(e)}{'（代理: ' + proxy + '），' if proxy else ''}跳过水龙头申请{Style.RESET_ALL}")
    except requests.exceptions.HTTPError as e:
        if e.response.status_code == 429:
            print(f"{Fore.RED}水龙头每天只能使用一次{'（代理: ' + proxy + '）' if proxy else ''}{Style.RESET_ALL}")
        elif e.response.status_code == 403:
            print(
                f"{Fore.RED}403 禁止访问：可能已达到速率限制或被阻止{'（代理: ' + proxy + '）' if proxy else ''}{Style.RESET_ALL}")
        else:
...
# 领取奖励
async def claim_reward(web3, private_key):
    account = Account.from_key(private_key)
    sender_address = account.address
    reward_contract = web3.eth.contract(address=rewardContractAddress, abi=rewardAbi)

    try:
        claim_method = reward_contract.functions.claim()
        nonce = web3.eth.get_transaction_count(sender_address)
        gas_estimate = claim_method.estimate_gas({'from': sender_address})
        tx = claim_method.build_transaction({
            'from': sender_address,
            'gas': int(gas_estimate * 1.2),
            'nonce': nonce
        })
        ...
        tx_hash = web3.eth.send_raw_transaction(signed_tx.rawTransaction)
        receipt = web3.eth.wait_for_transaction_receipt(tx_hash)
        print(f"{Fore.GREEN}成功领取奖励，交易哈希: {receipt.transactionHash.hex()}{Style.RESET_ALL}")
    except Exception as e:
        print(f"{Fore.RED}领取奖励时出错: {str(e)}{Style.RESET_ALL}")
...
async def process_wallets():
    ...
    if USE_PROXY:
        ...
        if len(private_keys) != len(proxies):
            print(
                f"{Fore.RED}错误：private_keys.txt（{len(private_keys)}行）与 proxy.txt（{len(proxies)}行）的行数不匹配{Style.RESET_ALL}")
            os._exit(1)
    else:
    ...
    for i, (private_key, proxy) in enumerate(zip(private_keys, proxies)):
        ...
        print(
            f"{Fore.GREEN}使用金额 {Fore.RED}{Web3.from_wei(random_bond_amount, 'ether')} ETH 创建 Bond Token（+4分）{Style.RESET_ALL}")
        ...
        print(
            f"{Fore.GREEN}使用金额 {Fore.RED}{Web3.from_wei(random_leverage_amount, 'ether')} ETH 创建 Leverage Token（+4分）{Style.RESET_ALL}")
        await perform_action(web3_instance, 'create', leverage_token_type, random_leverage_amount, min_amount,
                             private_key)

        # 赎回 Bond Token 50%
        print(f"{Fore.RED}正在赎回 Bond Token 余额的 50%...（+4分）{Style.RESET_ALL}")
        await perform_action(web3_instance, 'redeem', bond_token_type, random_bond_amount, min_amount, private_key)

        # 赎回 Leverage Token 50%
        print(f"{Fore.RED}正在赎回 Leverage Token 余额的 50%...（+4分）{Style.RESET_ALL}")
        await perform_action(web3_instance, 'redeem', leverage_token_type, random_leverage_amount, min_amount,
                             private_key)

        # 领取奖励
        print(f"{Fore.RED}正在为 {wallet_address} Claim领取质押...（+4分）{Style.RESET_ALL}")
        await claim_reward(web3_instance, private_key)

        print(f"{Fore.RED}=== 已完成钱包处理: {Fore.GREEN}{wallet_address}{Fore.RED} (总计20分) ===\n{Style.RESET_ALL}")

        # 如果不是最后一个钱包，添加随机延迟
        if i < len(private_keys) - 1:
            delay = random.uniform(1, 1)  # 生成 1-15 秒的随机数
...
def get_next_run_time():
    delay_in_hours = random.uniform(21, 21)
...
async def main():
    ...
    while True:
        ...
        print(
            f"{Fore.CYAN}{Style.BRIGHT}正在运行流程，时间: {datetime.now(UTC8).strftime('%Y-%m-%d %H:%M:%S')} (UTC+8){Style.RESET_ALL}")
        await process_wallets()
